...
def predict(network,x):
    ...
    return y
x,t=getdata()
#print(x.shape)#shape 返回的是一个从1开始的大小，len同理
network=init_network()


acc=0
batchsize=100
for i in range(0,len(x),batchsize):#range(start,end,step)will create a list ranging from start-->end-1
    y=predict(network,x[i:min(i+batchsize-1,len(x)-1)])
    p=np.argmax(y,axis=1)
    acc+=np.sum(p==t[i:min(i+batchsize-1,len(x)-1)])
